/**
 * Cron entrypoint — runs winback engine for ALL active tenants.
...
import { runWinbackForTenant } from "./engines.winback";
...
export const Route = createFileRoute("/hooks/engines/winback-all")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        for (const t of tenants ?? []) {
          try {
            const result = await runWinbackForTenant(t.id);
