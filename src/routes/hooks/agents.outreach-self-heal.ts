...
const PERMANENT_PATTERNS = [
  ...
  "deleted_by_moderator", "permanent_block", "spam_detected", "guard_reject",
];
...
async function runForTenant(tenantId: string) {
  ...
  for (const a of failed ?? []) {
    ...
    const { error } = await supabaseAdmin.from("outreach_actions").update({
  ...
  for (const ch of CHANNELS) {
    const [{ count: failedCnt }, { count: postedCnt }] = await Promise.all([
      supabaseAdmin.from("outreach_actions").select("*", { count: "exact", head: true })
        .eq("tenant_id", tenantId).eq("channel", ch).eq("status", "failed").gte("created_at", since24h),
      supabaseAdmin.from("outreach_actions").select("*", { count: "exact", head: true })
        .eq("tenant_id", tenantId).eq("channel", ch).eq("status", "posted").gte("created_at", since24h),
    ]);
    const f = failedCnt ?? 0;
    const p = postedCnt ?? 0;
    if (f >= 5 && p < 2) {
      const settingKey = `${ch}_posting_enabled`;
      const { data: setting } = await supabaseAdmin.from("outreach_settings")
        .select("value").eq("tenant_id", tenantId).eq("key", settingKey).maybeSingle();
      ...
      if (currentlyEnabled) {
        ...
        } as never, { onConflict: "tenant_id,key" });
