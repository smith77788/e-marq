/**
 * Cron entrypoint — runs abandoned cart engine for ALL active tenants.
...
import { runAbandonedCartForTenant } from "./engines.abandoned-cart";
...
export const Route = createFileRoute("/hooks/engines/abandoned-cart-all")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        for (const t of tenants ?? []) {
          try {
            const result = await runAbandonedCartForTenant(t.id);
