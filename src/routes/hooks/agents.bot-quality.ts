        ...
        try {
          ...
          if (outbound.length >= 20 && replyRate < 0.15) {
            insights.push({
              ...
              affected_layer: "bot",
              title: `Sales-бот: відповідей ${(replyRate * 100).toFixed(1)}% — ціль 15%`,
              description: `За ${WINDOW_DAYS} днів: надіслано ${outbound.length} повідомлень, відповіли протягом 24г лише ${replied}. Тон може бути занадто агресивним або шаблон надто загальним. Спробуйте м'якший вступ і питання, що запрошує до відповіді.`,
              expected_impact: `Підйом до 25% відповідей = ~${Math.round(outbound.length * 0.1)} нових розмов/тиждень`,
              confidence: 0.7,
              risk_level: "medium",
              metrics: {
          ...
          if (outbound.length >= 20 && conversionRate >= 0.08) {
            insights.push({
              ...
              affected_layer: "bot",
              title: `Sales-бот конвертує ${(conversionRate * 100).toFixed(1)}% — збільшити частоту`,
              description: `За ${WINDOW_DAYS} днів: ${customersContacted.size} клієнтів охоплено, ${conversions} зробили оплачене замовлення. Поточний playbook працює — розгляньте зниження порогу давності для більш ранього контакту з теплими лідами.`,
              expected_impact: `Подвоєння частоти охопить ~${Math.round(customersContacted.size * 0.5)} більше клієнтів/тиждень`,
              confidence: 0.8,
              risk_level: "low",
              metrics: {
                window_days: WINDOW_DAYS,
