/**
 * POST /hooks/agents/self-heal-apply
 * Body: { action_id: string }
 *
 * Applies a previously-proposed self-heal action. Super-admin only.
 */
import { createFileRoute } from "@tanstack/react-router";
import { createClient } from "@supabase/supabase-js";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import type { Database } from "@/integrations/supabase/types";
import { applyProposal } from "@/lib/self-heal/engine";
...
export const Route = createFileRoute("/hooks/agents/self-heal-apply")({
  server: {
    handlers: {
      POST: async ({ request }) => {
