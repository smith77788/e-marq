 ...
 *
 * Повертає повний дашборд для тенанта.
 *
...
import { getFullDashboard } from "@/lib/acos/dashboardSystem";
...
export const Route = createFileRoute("/api/dashboard/full")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const data = await getFullDashboard(tenantId);
