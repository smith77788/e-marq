/**
 * GET /api/data/monitoring?tenantId=xxx — get system health status
 *
 * Auth: Bearer JWT, roles owner / admin / super_admin.
 * Note: getSystemHealth() does not require tenantId; tenantId is used for auth only.
 */
import { createFileRoute } from "@tanstack/react-router";
import { createClient } from "@supabase/supabase-js";
import type { Database } from "@/integrations/supabase/types";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import { getSystemHealth } from "@/lib/acos/dataMonitoring";
...
export const Route = createFileRoute("/api/data/monitoring")({
  server: {
    handlers: {
      GET: async ({ request }) => {
