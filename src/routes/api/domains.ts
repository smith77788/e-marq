...
import { getDomainStatus, requestDomainVerification } from "@/lib/acos/domainVerification";
...
export const Route = createFileRoute("/api/domains")({
  ...
  async action({ request }) {
    ...
    try {
      const verification = await requestDomainVerification(tenantId, body.domain);
      return Response.json({ ok: true, verification });
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Failed to request verification";
      return err(msg, 500);
    }
  },
});
