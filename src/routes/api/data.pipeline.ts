 ...
 *
 * Обробка нових подій через data pipeline.
 * Body: { tenantId }
 *
...
import { processNewEvents } from "@/lib/acos/dataPipeline";
...
export const Route = createFileRoute("/api/data/pipeline")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const result = await processNewEvents(tenantId);
