/**
 * GET  /api/data/cleanup?tenantId=xxx — get storage stats
 * POST /api/data/cleanup               — cleanup old data
 *
 * Auth: Bearer JWT, roles owner / admin / super_admin.
 * Note: cleanupOldData(tenantId) accepts only tenantId; daysOld in the body is accepted
 * for API compatibility but the lib uses fixed retention periods internally.
 */
import { createFileRoute } from "@tanstack/react-router";
import { createClient } from "@supabase/supabase-js";
import type { Database } from "@/integrations/supabase/types";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import { getStorageStats, cleanupOldData } from "@/lib/acos/dataCleanup";
...
export const Route = createFileRoute("/api/data/cleanup")({
  server: {
    handlers: {
      GET: async ({ request }) => {
      ...
      POST: async ({ request }) => {
        let body: { tenantId?: string; daysOld?: number };
        try {
