/**
 * GET  /api/analytics/backup?tenantId=xxx  — історія метрик.
 * POST /api/analytics/backup               — зберегти snapshot метрик.
 *   Body: { tenantId, metrics }
 *
...
import { getMetricsHistory, saveMetricsSnapshot } from "@/lib/acos/analyticsBackup";
...
export const Route = createFileRoute("/api/analytics/backup")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const history = await getMetricsHistory(tenantId);
      ...
      POST: async ({ request }) => {
        ...
        const { tenantId, metrics } = body as {
          tenantId?: string;
          metrics?: Record<string, unknown>;
        };
        if (!tenantId) return err("tenantId required");
        if (!metrics || typeof metrics !== "object") return err("metrics required");

        const auth = await resolveAuth(request, tenantId);
        if (!auth.ok) return err(auth.error, auth.status);

        const result = await saveMetricsSnapshot(tenantId, metrics);
