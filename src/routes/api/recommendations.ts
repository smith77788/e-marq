/**
 * GET /api/recommendations?tenantId=xxx&customerId=xxx&productId=xxx&limit=6
 * — product recommendations for a tenant.
 *
...
import { getRecommendations } from "@/lib/acos/recommendationEngine";
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const recommendations = await getRecommendations(tenantId, customerId, productId, limit);
