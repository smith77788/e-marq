/**
 * GET  /api/leads?tenantId=xxx — get lead stats.
 * POST /api/leads — score a lead.
 *
...
import { getLeadStats, scoreLead } from "@/lib/acos/leadGeneration";
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const lead = body.lead as {
          email?: string;
          company?: string;
          source: string;
          interaction_count?: number;
        };
        if (!lead || !lead.source) return err("lead with source required");

        const scored = await scoreLead(tenantId, lead);
        return Response.json({ ok: true, lead: scored });
      },
    },
  },
});
