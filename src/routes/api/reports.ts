/**
 * GET /api/reports?tenantId=xxx&type=daily|weekly|monthly — generate a report
 *
...
import { generateReport } from "@/lib/acos/reportSystem";
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const rawType = u.searchParams.get("type") ?? "daily";
        const validTypes = ["daily", "weekly", "monthly"] as const;
        const type = validTypes.includes(rawType as (typeof validTypes)[number])
          ? (rawType as "daily" | "weekly" | "monthly")
          : "daily";

        const report = await generateReport(tenantId, type);
