/**
 * POST /api/shipping/options — варіанти доставки для замовлення.
 *
...
import { getShippingOptions, analyzeShippingCosts } from "@/lib/acos/shippingOptimizer";
...
export const Route = createFileRoute("/api/shipping/options")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        let body: unknown;
        ...
        const { tenantId, orderTotalCents, destinationCity } = parsed.data;

        const auth = await resolveAuth(request, tenantId);
        if (!auth.ok) return err(auth.error, auth.status);

        const [options, analysis] = await Promise.all([
          getShippingOptions(tenantId, orderTotalCents, destinationCity),
          analyzeShippingCosts(tenantId),
        ]);

        return Response.json({ ok: true, options, analysis });
      },
    },
  },
});
