 ...
 * Note: archiveOldData(tenantId) uses a fixed 90-day retention period internally;
 * olderThanDays in the body is accepted for API compatibility.
 */
import { createFileRoute } from "@tanstack/react-router";
import { createClient } from "@supabase/supabase-js";
import type { Database } from "@/integrations/supabase/types";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import { archiveOldData } from "@/lib/acos/dataArchiving";
...
export const Route = createFileRoute("/api/data/archive")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        let body: { tenantId?: string; olderThanDays?: number };
        try {
        ...
        try {
          const result = await archiveOldData(tenantId);
