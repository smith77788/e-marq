/**
 * POST /api/returns/risk — аналіз ризику повернення для замовлення.
 *
...
import { analyzeReturnRisk } from "@/lib/acos/returnManagement";
...
export const Route = createFileRoute("/api/returns/risk")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        let body: unknown;
        ...
        const risk = await analyzeReturnRisk(tenantId, orderId);
