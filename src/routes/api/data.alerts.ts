/**
 * GET /api/data/alerts?tenantId=xxx — check data anomalies
 *
...
import { checkDataAnomalies } from "@/lib/acos/dataAlerts";
...
export const Route = createFileRoute("/api/data/alerts")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const anomalies = await checkDataAnomalies(tenantId);
          return Response.json({ ok: true, data: anomalies });
        } catch (e) {
