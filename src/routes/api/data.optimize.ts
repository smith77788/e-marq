/**
 * GET /api/data/optimize
 *
 * Рекомендації з оптимізації запитів до БД.
 * ?tenantId=xxx
 *
...
import { getOptimizationRecommendations } from "@/lib/acos/dataOptimization";
...
export const Route = createFileRoute("/api/data/optimize")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const recommendations = await getOptimizationRecommendations(tenantId);
