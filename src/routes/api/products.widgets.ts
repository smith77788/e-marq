/**
 * GET /api/products/widgets?tenantId=xxx&type=best_sellers|new_arrivals|discounted
 * — product widget data for a tenant.
 *
...
import {
  ...
  getNewArrivalsWidget,
  getDiscountedWidget,
} from "@/lib/acos/productWidgets";
...
export const Route = createFileRoute("/api/products/widgets")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        switch (type) {
          case "new_arrivals":
            products = await getNewArrivalsWidget(tenantId);
            break;
          case "discounted":
            products = await getDiscountedWidget(tenantId);
            break;
          default:
            products = await getBestSellersWidget(tenantId);
        }
