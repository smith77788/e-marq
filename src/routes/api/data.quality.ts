/**
 * GET /api/data/quality?tenantId=xxx — analyze data quality
 *
...
import { analyzeDataQuality } from "@/lib/acos/dataQuality";
...
export const Route = createFileRoute("/api/data/quality")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const quality = await analyzeDataQuality(tenantId);
          return Response.json({ ok: true, data: quality });
        } catch (e) {
