/**
 * GET  /api/notifications/preferences?tenantId=xxx
 * POST /api/notifications/preferences
 *
 * Manages notification channel preferences for a tenant.
...
} from "@/lib/acos/notificationPreferences";
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const preferences = await getNotificationPreferences(tenantId);
      ...
      POST: async ({ request }) => {
        ...
        const frequency = (body.frequency as string) ?? "instant";

        if (!channel) return err("channel required");
        if (!type) return err("type required");
        if (typeof enabled !== "boolean") return err("enabled must be a boolean");

        const result = await updateNotificationPreference(tenantId, channel, type, enabled, frequency);
