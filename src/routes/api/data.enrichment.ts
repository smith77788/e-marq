/**
 * POST /api/data/enrichment — enrich customer data
 *
 * Auth: Bearer JWT, roles owner / admin / super_admin.
 * Note: enrichCustomerData(tenantId, limit) enriches up to `limit` customers.
 * The body accepts an optional `customerId` for API compatibility, but the lib
 * enriches in bulk by tenantId. Pass `limit` to control how many are processed.
 */
import { createFileRoute } from "@tanstack/react-router";
import { createClient } from "@supabase/supabase-js";
import type { Database } from "@/integrations/supabase/types";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import { enrichCustomerData } from "@/lib/acos/dataEnrichment";
...
export const Route = createFileRoute("/api/data/enrichment")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        let body: { tenantId?: string; customerId?: string; limit?: number };
        try {
        ...
        try {
          const enriched = await enrichCustomerData(tenantId, limit);
