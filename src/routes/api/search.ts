 ...
 * GET /api/search
 *
 * Пошукові можливості: автокомпліт, аналіз прогалин, оцінка якості.
 *
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          if (q !== null) {
            const suggestions = await getSearchSuggestions(tenantId, q);
            return Response.json({ ok: true, data: suggestions });
          }

          if (gaps === "true") {
            const gapsData = await analyzeSearchGaps(tenantId);
            return Response.json({ ok: true, data: gapsData });
          }

          if (quality === "true") {
            const score = await getSearchQualityScore(tenantId);
            return Response.json({ ok: true, data: score });
          }

          return err("Missing query param: q, gaps=true, or quality=true");
        } catch (e) {
