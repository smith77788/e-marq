 ...
 * GET /api/data/index
 *
 * Аналіз потреб у індексах для оптимізації БД.
 * ?tenantId=xxx
 *
...
import { analyzeIndexNeeds } from "@/lib/acos/dataIndexing";
...
export const Route = createFileRoute("/api/data/index")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const analysis = await analyzeIndexNeeds(tenantId);
