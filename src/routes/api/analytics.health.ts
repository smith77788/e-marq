/**
 * GET /api/analytics/health?tenantId=xxx — стан системи.
 *
...
import { performHealthCheck, getOverallHealth } from "@/lib/acos/healthCheck";
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const [checks, overall] = await Promise.all([
          performHealthCheck(),
          getOverallHealth(),
        ]);

        return Response.json({ ok: true, checks, overall });
      },
    },
  },
});
