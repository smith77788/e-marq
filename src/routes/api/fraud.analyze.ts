/**
 * POST /api/fraud/analyze — fraud risk analysis for an order.
 *
...
import { analyzeFraudRisk } from "@/lib/acos/fraudDetection";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const { tenantId, orderData } = parsed.data;

        const auth = await resolveAuth(request, tenantId);
        if (!auth.ok) return err(auth.error, auth.status);

        const risk = await analyzeFraudRisk(tenantId, orderData);
