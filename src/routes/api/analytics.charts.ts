...
import { getRevenueChart, getCustomerSegmentsChart } from "@/lib/acos/chartSystem";
...
export const Route = createFileRoute("/api/analytics/charts")({
  ...
  async loader({ request }) {
    ...
    if (type === "revenue") {
      const chart = await getRevenueChart(tenantId);
      return Response.json({ ok: true, chart });
    }

    if (type === "segments") {
      const chart = await getCustomerSegmentsChart(tenantId);
      return Response.json({ ok: true, chart });
    }

    // Default: return both in parallel
    const [revenue, segments] = await Promise.all([
      getRevenueChart(tenantId),
      getCustomerSegmentsChart(tenantId),
    ]);
    return Response.json({ ok: true, revenue, segments });
  },
});
