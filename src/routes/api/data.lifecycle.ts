 ...
 *
 * Стан життєвого циклу даних тенанта.
 * ?tenantId=xxx
 *
...
import { getDataLifecycle } from "@/lib/acos/dataLifecycle";
...
export const Route = createFileRoute("/api/data/lifecycle")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const data = await getDataLifecycle(tenantId);
