...
export function CustomerRoster({ tenantId }: Props) {
  const { data: customers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("customers")
        .select(
          "id, email, name, telegram_username, telegram_chat_id, lifecycle_stage, total_orders, total_spent_cents, last_order_at, predicted_next_order_at",
        )
  ...
  const totals = {
    ...
    overdue: customers.filter(
      (c) => c.predicted_next_order_at && new Date(c.predicted_next_order_at) < new Date(),
    ).length,
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Кого система знає та що для них прогнозує.</CardDescription>
        ...
        {isLoading ? (
          ...
          <div className="rounded-md border border-dashed border-border bg-muted/20 p-6 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Покупці зʼявляться тут автоматично, коли надійде перша оплата.
            </p>
          </div>
        ) : (
                ...
                return (
                  <DetailableElement
                    ...
                    drawerTitle={
                      c.name ??
                      c.email ??
                      (c.telegram_username ? `@${c.telegram_username}` : "Анонім")
                    }
                  ...
                  >
                    <div className="rounded-lg border border-border bg-card p-3">
                      <div className="flex items-center gap-2">
                        {c.lifecycle_stage === "vip" && (
                          <Crown className="h-3.5 w-3.5 text-warning-foreground" />
                        )}
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-foreground">
                            {Math.round(c.total_spent_cents / 100).toLocaleString("uk-UA")} ₴
                          </p>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Наступна покупка</span>
                          <p
                            className={`font-medium ${overdue ? "text-destructive" : "text-foreground"}`}
                          >
                            {c.predicted_next_order_at
                              ? new Date(c.predicted_next_order_at).toLocaleDateString("uk-UA")
                              : "—"}
                          </p>
                        </div>
                      </div>
...
function Mini({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div
      className={`rounded-md border p-2 ${highlight ? "border-primary/30 bg-primary/5" : "border-border bg-card"}`}
    >
      <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
        {label}
      </p>
      <p className={`mt-0.5 text-base font-bold ${highlight ? "text-primary" : "text-foreground"}`}>
        {value}
      </p>
    </div>
  );
