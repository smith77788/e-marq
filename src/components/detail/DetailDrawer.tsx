  ...
  return (
    <Tabs defaultValue={defaultTab} className="w-full">
      <TabsList className="grid w-full grid-cols-4">
        <TabsTrigger value="overview" disabled={!hasOverview}>
          Огляд
        </TabsTrigger>
        <TabsTrigger value="details" disabled={!hasDetails}>
          Деталі
        </TabsTrigger>
        <TabsTrigger value="log" disabled={!hasLog}>
          Лог
        </TabsTrigger>
        <TabsTrigger value="actions" disabled={!hasActions}>
          Дії
        </TabsTrigger>
      </TabsList>
        ...
        {data.metrics && data.metrics.length > 0 && (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {data.metrics.map((m, i) => (
              <div key={i} className={cn("rounded-lg border p-3", toneClasses(m.tone))}>
            ...
            {data.ai_insights.map((ins) => (
              <div
                ...
                className={cn(
                  ...
                  toneClasses(ins.tone === "info" ? "primary" : ins.tone),
                )}
              >
                ...
                {typeof ins.confidence === "number" && (
                  <p className="mt-1 text-[10px] opacity-70">
                    Впевненість: {Math.round(ins.confidence * 100)}%
                  </p>
                )}
              </div>
            ))}
              ...
              <tbody>
                {Object.entries(data.metadata).map(([k, v]) => (
                  <tr key={k} className="border-b border-border last:border-0">
                    <td className="w-1/3 px-3 py-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                      {k}
                    </td>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {data.related_items.map((r) => (
                ...
                >
                  ...
                  {r.subtitle && (
                    <p className="mt-0.5 truncate text-xs text-muted-foreground">{r.subtitle}</p>
                  )}
...
function DetailContents() {
  ...
  const primaryAction =
    query.data?.actions?.find((a) => a.variant === "primary") ?? query.data?.actions?.[0];
    ...
    return (
      ...
      >
        <DrawerContent className="max-h-[90vh]">
          ...
          <div className="min-h-0 flex-1 overflow-hidden">{open && <DetailContents />}</div>
        </DrawerContent>
      </Drawer>
    );
  }
