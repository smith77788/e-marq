    ...
    mutationFn: async () => {
      ...
      const { data, error } = await withTimeout(
        supabase.rpc("save_tenant_integration", {
          ...
          _credentials: payload.credentials_encrypted ?? undefined,
          _config: payload.config,
          _last_sync_status: payload.last_sync_status ?? undefined,
          _last_sync_error: payload.last_sync_error ?? undefined,
          _webhook_secret: payload.webhook_secret ?? undefined,
        }),
        INTEGRATION_UI_TIMEOUT_MS,
          ...
          <div className="flex gap-2">
            ...
            {step === 1 && (isApiKey || isRest || isSheets || isWebhook) && (
              <Button
                ...
                title={
                  verified
                    ? "Перевірено — можна зберігати"
                    : "Зберегти підключення навіть без успішної перевірки"
                }
              >
                {saveConn.isPending ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
