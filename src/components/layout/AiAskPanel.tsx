 ...
 * відповідь та suggestions як натиснені CommandItem-и.
 */
...
type Props = {
  ...
  question: string;
  /** Викликається коли користувач натискає suggestion. */
  onNavigate: (to: string) => void;
};

export function AiAskPanel({ tenantId, question, onNavigate }: Props) {
  const { t } = useT();
  ...
  return (
    <>
      <CommandGroup heading={t("gs.aiHeading")}>
        ...
        {!loading && !error && !result && question.trim().length < 3 && (
