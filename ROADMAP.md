...
1. ~~Платіжні credentials UI~~ ✅ + ~~закрити витік секретів~~ ✅ (зроблено)
...
7. **Оплата підписки** — `owner_change_plan` досі міняє план без списання коштів. (розділ 1)

> Зроблено 2026-06-12: кольори бренду на вітрині, редактор hero+оголошення,
> реальний `winback_touch`; **платіжний UI для власника + закриття витоку
> платіжних секретів** (whitelist у `get_storefront_config`). Деталі — git history.
