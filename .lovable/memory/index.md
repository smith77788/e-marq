...
- [Cohort Retention Drop Detector](mem://features/cohort-retention-drop-detector) — SQL agent #23, daily 04:15, m1/m3 vs медіана попередніх 6 когорт, drop≥30%/40% → cohort_retention_drop owner_review
- [Discount Effectiveness Monitor](mem://features/discount-effectiveness-monitor) — SQL agent #24, daily 06:30, margin-aware post-mortem знижок; actual<0 → discount_loss_maker (high), <50% expected → discount_underperforming (medium); owner_review_rules
- [Telegram bot transport](mem://features/telegram-polling) — WEBHOOK mode на /api/public/telegram/webhook (secret=sha256, idempotency через telegram_processed_updates), polling cron вимкнений; latency <1с; setWebhook + rollback procedure всередині
