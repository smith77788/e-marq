...
BEGIN
  ...
  VALUES ('sql_pipeline', NULL, current_date, 1,
    CASE WHEN v_score < 1.0 THEN 1 ELSE 0 END, v_score,
    jsonb_build_object('issues', v_issues, 'last_insight', v_last_insight, 'last_decision', v_last_decision, 'last_outcome', v_last_outcome, 'pending_insights', v_pending_insights, 'stale_pending', v_stale_pending_decisions, 'checked_at', now()),
    now())
  ...
  RETURN jsonb_build_object('score', v_score, 'issues', v_issues);
END;
$$;