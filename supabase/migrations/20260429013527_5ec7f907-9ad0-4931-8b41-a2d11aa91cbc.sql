...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO action_outcomes (
      ...
      '7d_before_vs_after',
      now(),
      'auto-measured by measure_pending_outcomes()'
    );
...
SELECT cron.unschedule('measure-action-outcomes-6h')
WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'measure-action-outcomes-6h');

SELECT cron.schedule(
  'measure-action-outcomes-6h',
  '17 */6 * * *',
