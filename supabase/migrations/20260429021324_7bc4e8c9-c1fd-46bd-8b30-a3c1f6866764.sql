...
BEGIN
  ...
  LOOP
    ...
    -- скільки done-дій тенанта потрапили у вікно ±3 дні навколо цієї
    ...
    IF d.existing_outcome_id IS NOT NULL THEN
      UPDATE action_outcomes SET
        ...
        measurement_window = 'demo_pre3d_vs_postNd_split',
        measured_at = now(),
        notes = format('demo split 1/%s of tenant delta', v_share_denom)
    ...
    ELSE
      ...
      VALUES (d.tenant_id, d.id, d.executor_action_id, d.agent_id, d.action_type,
        ...
        'demo_pre3d_vs_postNd_split', now(),
        format('demo split 1/%s of tenant delta', v_share_denom));
    END IF;
