-- Fix measurement attribution: видалити overcounted записи + переробити функцію
-- так, щоб вона завжди ділила tenant-delta пропорційно по N done-діям у вікні,
-- і працювала навіть для свіжих дій (executed_at > now()-7d) — для синтетичного pilot.

-- 1) Видалити старі некоректні рядки (без split → tenant-delta задвоєний по N actions)
DELETE FROM public.action_outcomes WHERE measurement_window = '7d';

-- 2) Оновити функцію: прибрати "executed_at < now() - 7 days", замінити на
--    "executed_at <= now() - interval '1 hour'" — даємо хоч 1h на ефект.
--    Window post: min(7d, now()-executed_at). Якщо <1h — пропускаємо.
...
BEGIN
  FOR d IN
    ...
    WHERE dq.status = 'done'
      ...
      AND dq.executed_at <= now() - interval '1 hour'
      AND dq.executed_at >  now() - interval '30 days'
      AND ao.id IS NULL
    LIMIT 500
  LOOP
    -- адаптивне вікно: max 7d, min 1h
    ...
    -- proportional split: всі done-дії того ж tenant у ±3d від цієї дії
    ...
    INSERT INTO action_outcomes (
      ...
      jsonb_build_object('revenue_cents', v_delta_revenue,    'orders_count', v_delta_orders,
                         'share_denom', v_share_denom),
      v_attrib,
      v_is_success,
      'adaptive_pre_vs_post_split',
      now(),
