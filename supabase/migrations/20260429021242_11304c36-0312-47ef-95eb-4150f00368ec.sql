...
BEGIN
  FOR d IN
    ...
    WHERE dq.status = 'done'
      ...
      AND dq.executed_at < now() - interval '7 days'
  ...
  LOOP
    ...
    WHERE tenant_id = d.tenant_id
      ...
      AND created_at >= d.executed_at - interval '7 days'
    ...
    -- Скільки done-дій того ж тенанта потрапили в той самий 7-денний window.
    -- Це знаменник для proportional attribution.
    ...
    INSERT INTO action_outcomes (
      ...
      v_attrib,
      v_is_success,
      '7d_before_vs_after_split',
      now(),
      format('proportional split: 1/%s of tenant delta', v_share_denom)
    );
...
-- Очищаю старі завищені outcomes (зроблені попередньою версією функції) — їх перерахує tick.
DELETE FROM public.action_outcomes
 WHERE measurement_window = '7d_before_vs_after'
