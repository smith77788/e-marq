...
-- Phase 17 fix: tenants.id (not tenant_id)
...
DECLARE
  ...
  v_avg_minutes_per_action int := 8;
BEGIN
  FOR v_tenant IN
    SELECT t.id AS tenant_id, t.slug, tc.owner_telegram_chat_id
    FROM tenants t
  ...
  LOOP
    ...
    SELECT d.action_type, d.title, ao.attributed_revenue_cents
    INTO v_top_win
    ...
    VALUES (
      v_tenant.tenant_id, 'digest', v_chat_id, 'pending',
      jsonb_build_object('text', v_msg, 'parse_mode', 'Markdown', 'disable_web_page_preview', true)
    );
...
-- Validate (skip 08:00 cron run today since Phase 11 already ran the broken-old version at 08:00)
SELECT public.send_owner_daily_digest();
