...
BEGIN
  ...
  LOOP
    ...
    v_sem_key := public._decision_semantic_key(v_action, v_payload, i.insight_type);
    ...
    IF EXISTS (
      SELECT 1 FROM public.decision_queue dq
       ...
       WHERE dq.tenant_id = i.tenant_id
         ...
         AND public._decision_semantic_key(dq.action_type, dq.payload, ai2.insight_type) = v_sem_key
    ) THEN
      UPDATE public.ai_insights SET status='applied', updated_at=now() WHERE id=i.id;
      v_skipped := v_skipped + 1;
      CONTINUE;
