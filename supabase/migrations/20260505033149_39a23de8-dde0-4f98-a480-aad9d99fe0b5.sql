...
BEGIN
  WITH velocity AS (
    ...
    WHERE o.created_at > now() - interval '30d' AND o.status IN ('paid','fulfilled')
    GROUP BY oi.tenant_id, oi.product_id
  ),
  ranked AS (
    SELECT v.*, p.name AS product_name,
      CASE WHEN v.units_prior_23d > 0 THEN v.units_7d / (v.units_prior_23d / 23.0 * 7.0) ELSE NULL END AS ratio
    FROM velocity v JOIN products p ON p.id = v.product_id AND p.tenant_id = v.tenant_id
  ...
  FROM ranked r WHERE r.ratio >= 1.5
    AND NOT EXISTS (SELECT 1 FROM ai_insights ai WHERE ai.tenant_id = r.tenant_id
        AND ai.insight_type = 'trending_product' AND ai.metrics->>'product_id' = r.product_id::text
