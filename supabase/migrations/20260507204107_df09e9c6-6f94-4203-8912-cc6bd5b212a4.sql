...
BEGIN
  WITH candidates AS (
    SELECT l.tenant_id, l.customer_id, l.predicted_ltv_cents, l.churn_probability, l.segment, c.email, c.name
  ...
  inserted AS (
    INSERT INTO ai_insights (tenant_id, insight_type, affected_layer, title, description, confidence, risk_level, status, metrics, dedup_bucket)
    SELECT
      cd.tenant_id, 'high_ltv_at_risk', 'customer',
      ...
      'new',
      jsonb_build_object('action','priority_winback','segment',cd.segment,'predicted_ltv_cents',cd.predicted_ltv_cents,'churn_probability',cd.churn_probability,'customer_id',cd.customer_id),
