...
DECLARE
  ...
  v_predicted bigint;
BEGIN
  SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY mult)
  INTO v_global_mult
  FROM (
    SELECT (
  ...
  LOOP
    ...
    SELECT
      percentile_cont(0.5) WITHIN GROUP (ORDER BY mult),
      COUNT(*) FILTER (WHERE mult IS NOT NULL AND mult > 0)
    ...
    IF COALESCE(v_tenant_n, 0) >= 6 AND v_tenant_mult IS NOT NULL THEN
      v_mult := v_tenant_mult; v_source := 'tenant'; v_confidence := 'high';
    ELSIF COALESCE(v_tenant_n, 0) >= 3 AND v_tenant_mult IS NOT NULL THEN
...
BEGIN
  ...
  LOOP
    ...
    SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY predicted_ltv_12m_cents)
    INTO v_baseline_median
    FROM (
