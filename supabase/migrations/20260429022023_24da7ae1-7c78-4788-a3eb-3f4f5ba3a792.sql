...
BEGIN
  ...
  WHERE pm.tenant_id = _tenant_id AND pm.units_sold >= 5 AND pm.current_stock > 0
    AND (pm.current_stock::numeric / pm.units_sold * 14.0) <= 7
  ...
  -- 6. Conversion drop — обчислюємо в CTE з guard
  WITH cur AS (
  ...
  guarded AS (
    SELECT cur.paid cpaid, cur.visits cvis, cur.rev crev, prev.paid ppaid, prev.visits pvis, prev.rev prev
    FROM cur, prev
    WHERE prev.paid >= 5 AND prev.visits >= 50
      AND (cur.paid::numeric / prev.paid) <= 0.6
      AND (cur.visits::numeric / prev.visits) >= 0.7
  )
  INSERT INTO ai_insights (tenant_id, insight_type, affected_layer, title, description, expected_impact, confidence, risk_level, status, metrics, dedup_bucket)
  SELECT _tenant_id, 'conversion_drop', 'funnel',
    'Конверсія впала на ' || ROUND((1 - g.cpaid::numeric/g.ppaid)*100) || '%',
    'За останні 7 днів оплачених замовлень: ' || g.cpaid || ' (попередні 7 днів: ' || g.ppaid || '). Трафік: ' || g.cvis || ' vs ' || g.pvis || '.',
    'Потенційна втрата ~' || ((g.prev - g.crev)/100) || ' ₴ за тиждень',
    0.82, 'high', 'new',
    jsonb_build_object('paid_now', g.cpaid, 'paid_prev', g.ppaid, 'visits_now', g.cvis, 'visits_prev', g.pvis, 'revenue_delta_cents', g.prev - g.crev),
